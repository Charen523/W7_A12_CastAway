        ...
        foreach (GameObject obj in pool)
        {
            ...
            while (!validPosition)
            {
                ...
                if (Physics.Raycast(new Vector3(randomX, initialY, randomZ), Vector3.down, out hit, Mathf.Infinity, terrainLayerMask))
                {
                    ...
                    randomPosition = new Vector3(randomX, hit.point.y + (carrotHeight * carrotVisibleHeight), randomZ);
                }
                else
                {
                    // Raycast가 실패할 경우 안전장치로 positionMin.y 사용
                    randomPosition = new Vector3(randomX, positionMin.y + (randomScale.y * carrotVisibleHeight), randomZ);
                }
